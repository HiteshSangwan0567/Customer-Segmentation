...
print('percent_missing_per_column', percent_missing_per_column)
...
# 
# As we can see in the percent_missing_per_column data most of the coulmn have less than 20% of the data missing. The columns with more 20% missing data are called as outlier column and are dropped from the database :
...
# The data charts clearly indicates the the distribution of values between the two datasets are similar for the Gender features, but it is not valid for the financial datasets where we can see massive differences in missing data. 
# 
...
# Re-encode categorical variable(s) to be kept in the analysis.
new_value = {'W': 0, 'O': 1}
azdias_clean = azdias_lt_3.replace({'OST_WEST_KZ':new_value})
...
# This Re-Encoding required some major overhauls:
# 1. Re-Encoding the values in 'OST_WEST_KZ' with 'W' and 'O' replaced by '0' and '1' respectively. This made it uselful for us in our analysis because we can use it now rather than just dropping it as a whole.
# 2. All the binary features ['ANREDE_KZ', 'GREEN_AVANTGARDE', 'SOHO_KZ', 'VERS_TYP', 'OST_WEST_KZ'] are kept where as all the multi-level features are droppped. 
...
# 
# The best mixed-value features in the dataset are:
# 1. PRAEGENDE_JUGENDJAHRE
# 2. CAMEO_INTL_2015
# These features can not be included directly for analyzing....so we have to engineer our way out :
# 1. Create two new feature columns
# 2. Copy values from the initial mixed-value features
# 3. Create two dictionaries to map the initial feature values to the brand new feature values.
# 4. Implement a replace operation using the dictionaries on the new feature columns.
...
# 
# The missing values in the dataset are replaced by the most frequently{Imputer(strategy='most_frequent')} used values using the Imputer method and then all the features ar scaled using the StandardScaler.(scaler = StandardScaler())
...
# Here the no of principal components retained is 30, as it reduces the no of features by 50% hence saving much needed inveestment in time and space and also nearly explains about 90% of the variance.
...
# 2.PLZ8_ANTG3 and WEALTH also have a positive correlation. It indicates that when the share of 6-10 family homes in a macro-cell increases, it tends to have more poor households.
# 3.MOBI_REGIO and PLZ8_ANTG3 have a negative correlation. It means that when the share of 6-10 family homes in a macro-cell increases, it tends to have higher movements.
# 
# From the second principal component, it can be seen that
# 
# 1. ALTERSKATEGORIE_GROB SEMIO_ERL have a positive correlation about the age and the behaviour of a human being.
# 
...
# 
# 1. CLustering using the Sklearn KMeans on the data.
# 2. Computing the averages within cluster distances. 
# 3. Plotting the curve 
# 4. K = 14  seems to be the elbow and hence the no of clusters for this problem is 14.
...
# 
# 1. In areas where the share of 6-10 family homes is lower (PLZ8_ANTG3=1.73)
# 2. In Prosperous or Comfortable households (WEALTH=2.75)
# 3. In life stage of Families With School Age Children or Older Families & Mature Couples (LIFE_STAGE=3.30)
# 
...
# 
# 1. In areas where the share of 6-10 family homes is higher (PLZ8_ANTG3=2.44)
# 2. In Less Affluent or Poorer households (WEALTH=4.4)
# 3. In life stage of Pre-Family Couples & Singles or Young Couples With Children (LIFE_STAGE=1.98)
# 
